...
<script type="text/javascript">
	...
	function deleteByIds(id) {
		...
		//按钮
			...
			window.location.href = "workdodel?id=" + id;
	...
	<div class="bodywrapper">
		...
		<div class="centercontent tables">
			...
			<div class="pageheader notab">
				<h1 class="pagetitle">零工列表</h1>
				<span class="pagedesc">查询所有的零工</span>
			...
			<div id="contentwrapper" class="contentwrapper">
				...
				<form action="work_list" method="post">
					<ul class="seachform1">

						<li><label>零工标题</label><input name="keyword1" type="text"
							class="scinput1" value="<%=keyword1%>" style="width: 216px; " /></li>

						<li><label>零工类目</label>
							<div class="vocation">
								<select name="keyword2" class="select3">
									<option value="0">请选择类目...</option>
									<c:forEach var="wtype" items="${requestScope.wtlist }">

										<c:choose>
											<c:when test="${wtype.id==requestScope.keyword2 }">
				...
				<table cellpadding="0" cellspacing="0" border="0" class="stdtable"
					...
					<thead>
						<tr>
							<th class="head0">零工标题</th>
							<th class="head1">零工类型</th>
							<th class="head0">图片</th>
							<th class="head1">佣金</th>
							<th class="head0">用户名</th>
							<th class="head1">发布者电话</th>
							<th class="head0">招募人数</th>
					...
					<tbody>
						...
						<%
							...
							int pageNo = Integer.valueOf(request.getAttribute("pageNo")
									.toString());
							int pageSize = Integer.valueOf(request.getAttribute("pageSize")
									.toString());
							int totalPages = Integer.valueOf(request.getAttribute("totalPages")
									.toString());
							int totalRecords = Integer.valueOf(request.getAttribute(
						...
						<c:forEach var="work" items="${requestScope.worklist}">
							<tr class="gradeX">
								...
								<td><img src="${work.img }" alt="" width="60px"
									height="60px" /></td>
								<td>${work.price }</td>
								...
								<td>${work.dTime }</td>
								<td>${work.content }</td>
								<td><a href="javascript:openEditView(${work.id });"
									class="tablelink">修改</a> <a
									href="javascript:deleteByIds(${work.id });" class="tablelink">
										删除</a></td>
							</tr>
						</c:forEach>
				...
				<div class="pagin">
					<div class="message">
						共<i class="blue"><%=totalRecords%></i>条记录，当前显示第&nbsp;<i
							class="blue"><%=pageNo%>&nbsp;</i>页&nbsp;&nbsp; 共&nbsp;<i
							class="blue"><%=totalPages%>&nbsp;</i>页
					</div>
					<ul class="paginList">
						<!-- 设置分页导航 -->
						<%
						...
						<li class="paginItem"><a
							href="goods_list?pageNo=<%=pageNo + 1%>&keyword1=<%=keyword1%>&keyword2=<%=keyword2%>"><span
								class="pagenxt"></span></a></li>
						<li class="paginItem"><a
							href="goods_list?pageNo=<%=totalPages%>&keyword1=<%=keyword1%>&keyword2=<%=keyword2%>">末页</a></li>
						...
						%>
						<li class="paginItem"><a
							href="goods_list?pageNo=1&keyword1=<%=keyword1%>&keyword2=<%=keyword2%>">首页</a></li>
						<li class="paginItem"><a
							href="goodst_list?pageNo=<%=pageNo - 1%>&keyword1=<%=keyword1%>&keyword2=<%=keyword2%>"><span
